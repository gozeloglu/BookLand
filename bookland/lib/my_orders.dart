...
class MyOrders extends StatelessWidget {
  ExploreStateless(int bookId) {
    ...
    if (orderIDset.contains(deletedBookId)) {
      orderIDset.remove(deletedBookId);
    }
  }
...
class MyOrdersState extends State<MyOrdersPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyAppBar(pageTitle: "My Orders", back: true,filter_list:true ),
  ...
  List<dynamic> listBooksGetter(OrderData booksData) {
...
class OrderData {
  ...
  OrderData.fromResponse(http.Response response) {
    ...
    for (int i = 0; i < jsonData.length; i++) {
      ...
      orderIDset.add(jsonData[i]["bookId"]);
